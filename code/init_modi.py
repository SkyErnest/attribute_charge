...
def load_data_and_labels_fewshot():
	...
	while True:
		...
		# 	content = content[1:]
		# except ValueError:
		# 	word2id[content[0]+' '+content[1]] = len(word2id)
		# 	content = content[2:]
		word2id[content[0]] = len(word2id)
		content = content[1:]
	...
	word_embeddings.append(lists)
	word_embeddings = np.array([i for i in word_embeddings],dtype='float32')
	print('word_embedding init',len(word_embeddings))
	# import pickle
	# pickle.dump(word2id,open('w2id.pkl','wb'))

	relationhash = {}
	namehash = {}
	num_r = {}
	farticlemap = open(law_attribution_path,'r',encoding='utf-8')
	...
	# ==================================================================
	# ADDED
	tfidf=TfidfVectorizer(max_features=tfidf_vec_len)
	tfidf_vec_train = tfidf.fit_transform(x_train).toarray()
	tfidf_vec_val = tfidf.transform(x_val).toarray()
	tfidf_vec_test = tfidf.transform(x_test).toarray()
	...
	print('init finish')
	return word2id,word_embeddings,attr_table,x_train,y_train,y_attr_train,tfidf_vec_train,x_test,y_test,y_attr_test,tfidf_vec_test,x_val,y_val,y_attr_val,tfidf_vec_val,namehash,length_train,length_test,length_val
